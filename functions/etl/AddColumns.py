...
class AddColumnsOperation(object):
    ...
    def transform(self, df1, df2, balanced, sufixes, numFrag):
        """AddColumnsOperation.
        ...
        """
        indexes1, indexes2, len1, len2 = self.check_indexes(df1, df2, numFrag)
        ...
        if op:
            for f in range(numFrag):
                for j in range(numFrag):
                    if self.is_overlapping(indexes1[f], indexes2[j]):
                        last = (f == numFrag-1) or (j == numFrag-1)
                        result[f] = self._add_columns(result[f], df1[f],
                                                      df2[j], indexes1[f],
                                                      indexes2[j], sufixes,
                                                      last, op)
                        startfill = f
            ...
            if startfill < numFrag-1:
                for f in xrange(startfill+1, numFrag):
                    result[f] = self._add_columns_fill(df1[f], len2,
                                                       op, sufixes)
        else:
            for f in range(numFrag):
                for j in range(numFrag):
                    if self.is_overlapping(indexes1[j], indexes2[f]):
                        last = (f == numFrag-1) or (j == numFrag-1)
                        result[f] = self._add_columns(result[f], df1[j],
                                                      df2[f], indexes1[j],
                                                      indexes2[f], sufixes,
                                                      last, op)
                        startfill = f
            ...
            if startfill < numFrag-1:
                for f in xrange(startfill+1, numFrag):
                    result[f] = self._add_columns_fill(df2[f], len1,
                                                       op, sufixes)

        return result


    def check_indexes(self, df1, df2, numFrag):
        ...
        len1 = [self.len_count(df1[f]) for f in range(numFrag)]
        len1 = mergeReduce(mergeCount, len1)
        len2 = [self.len_count(df2[f]) for f in range(numFrag)]
        ...
        for f in range(numFrag):
            indexes1[f] = self.reindexing(len1, f)
            indexes2[f] = self.reindexing(len2, f)
        return indexes1, indexes2, len1, len2


    @task(returns=list)
    def _add_columns(self, c, a, b,  idxs1, idxs2, suffixes, last, side):
    ...
    @task(returns=list)
    def _add_columns_fill(self, df1, info2, op, suffixes):
    ...
    def is_overlapping(self, indexes1, indexes2):
    ...
    def reindexing(self, len1, index):
