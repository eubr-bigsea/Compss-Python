        ...
        if TYPE == "right":
            for f1 in range(numFrag):
                for f2 in range(numFrag):
                    last = (f2 == numFrag-1)
                    result[f1] = Join(result[f1], data2[f1], data1[f2],
                                      params, f2, last)
        else:
            for f1 in range(numFrag):
                for f2 in range(numFrag):
                    last = (f2 == numFrag-1)
                    result[f1] = Join(result[f1], data1[f1], data2[f2],
                                      params, f2, last)
    else:
        sortIdx1 = [[] for i in range(numFrag)]
        sortIdx2 = [[] for i in range(numFrag)]
        for i in range(numFrag):
            sortIdx1[i] = Join_PartialSort(data1[i], key1)
            sortIdx2[i] = Join_PartialSort(data2[i], key2)
        ...
        if TYPE == "right":
            ...
            for f1 in range(numFrag):
                ...
                for f2 in range(numFrag):
                    ...
                    if over:
                        result[f1] = Join(result[f1], data2[f1],
                                          data1[f2], params, f2, last)
                        filled = True
                if not filled:
                    result[f1] = fixColumns(data2[f1], sortIdx1[0], params)
        else:
            ...
            for f1 in range(numFrag):
                ...
                for f2 in range(numFrag):
                    over, last = overlapping[f1][f2]
                    if over:
                        result[f1] = Join(result[f1], data1[f1],
                                          data2[f2], params, f2, last)
                        filled = True
                if not filled:
                    result[f1] = fixColumns(data1[f1], sortIdx2[0], params)

    return result


def RenameCols(cols1, cols2, key, suf, keep, op):
...
def Join(result, data1, data2, params, index, last):
    ...
    convert1, key1 = RenameCols(cols1, cols2, key1, LSuf, keep, True)
    data1 = data1.rename(columns=convert1)
    convert2, key2 = RenameCols(cols2, cols1, key2, RSuf, keep, False)
...
@task(returns=list)
def overlap(sortIdx1, sortIdx2, numFrag):
...
@task(returns=list)
def fixColumns(data1, cols2, params):
