...
def kmeans(data, k, numFrag, maxIterations, epsilon, initMode):
    """ 
        ...
        :param data: A np.array (splitted or not)
    ...
    if numFrag == -1:
        ...
        # fragment data
        size = int(math.ceil(float(len(data))/numFrag))
        data = [d for d in chunks(data,size) ]
    ...
    while not has_converged(mu, oldmu, epsilon, n, maxIterations):
        oldmu = list(mu)
        clusters = [cluster_points_partial(data[f], mu, f * size) for f in range(numFrag)]
        partialResult = [partial_sum(data[f], clusters[f], f * size) for f in range(numFrag)]
...
if __name__ == "__main__":
    ...
    filename = "/home/lucasmsp/workspace/BigSea/Benchmark-COMPSs_SPARK/Java_COMPSs/Datasets/higgs/train_0.12m.csv"
    k=2
    numFrag=4
    maxIterations=15
    epsilon=1e-4
    ...
    data = read_file_vector(filename,",")

    #print data


    mu = kmeans(data, k, numFrag, maxIterations, epsilon, initMode)
