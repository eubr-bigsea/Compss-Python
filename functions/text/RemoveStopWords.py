...
class RemoveStopWordsOperation(object):
    ...
    def transform(self, data, stopwords, settings, numFrag):
        """
        ...
        for f in range(numFrag):
            stopwords[f] = self.read_stopwords(stopwords[f], settings)
        stopwords = mergeReduce(self.merge_stopwords, stopwords)

        result = [[] for f in range(numFrag)]
        for f in range(numFrag):
            result[f] = self._remove_stopwords(data[f], settings, stopwords)
        return result

    @task(returns=list)
    def read_stopwords(self, data1,settings):
        if len(data1)>0:
    ...
    @task(returns=list)
    def merge_stopwords(self, data1, data2):
        data1 = np.concatenate((data1, data2), axis=0)
        return data1


    @task(returns=list)
    def _remove_stopwords(self, data, settings, stopwords):
