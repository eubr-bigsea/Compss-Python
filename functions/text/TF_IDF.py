...
class TF_IDF(object):
    ...
    def fit(self, data, params, numFrag):
        ...
        result_p = [wordCount(data[f], params) for f in range(numFrag)]
    ...
    def transform(self, test_set, model, params, numFrag):
        ...
        counts = [count_records(test_set[f]) for f in range(numFrag)]
...
def filter_words(vocabulary, params):
    ...
    if size > 0:
        vocabulary = vocabulary.sort_values(
            ['DistinctFrequency', 'TotalFrequency'], ascending=[1, 1]).\
            head(size)
...
def construct_TF_IDF(data, vocabulary, params, num_doc):
    ...
    alias = params['alias']
    columns = params['attributes']
    vector = np.zeros((data.shape[0], vocabulary.shape[0]), dtype=np.int)
