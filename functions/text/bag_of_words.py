...
class BagOfWords(object):
    """Bag-of-words's methods.

    - fit():
    - transform():
    """

    def fit(self, train_set, params, nfrag):
        """Fit.
        ...
        params['size'] = params.get('size', 1000)
    ...
    def transform(self, test_set, model, params, nfrag):
        ...
        result_p = [[] for _ in range(nfrag)]
        for f in range(nfrag):
            result_p[f] = _transform_BoW(test_set[f], vocabulary, params)

        return result_p

    def transform_serial(self, test_set, model, params):
        algorithm = model.get('algorithm', '')
        if algorithm != 'BagOfWords':
            raise Exception("You must inform a valid BagOfWords model.")
        vocabulary = model['model']
        ...
        return _transform_BoW_(test_set, vocabulary, params)
...
def _transform_BoW(data, vocabulary, params):
    return _transform_BoW_(data, vocabulary, params)


def _transform_BoW_(data, vocabulary, params):
    """Transform the dataset into bow's features."""
    alias = params['alias']
    columns = params['attributes']
