...
def generate_partition(size, cols, dim, frag):
    ...
    df = pd.DataFrame(np.random.normal(size=[size, dim]),
                      columns=cols)
...
if __name__ == "__main__":
    ...
    n_components = 2
    new_cols = ["pca{}".format(c) for c in range(n_components)]
    pca = PCA(input_col=cols, n_components=n_components,
              output_col=new_cols, remove=True).fit(ddf1)
    t3 = time.time()
    print("Time to PCA.fit (t3-t2):", t3 - t2)

    ddf1 = pca.transform(ddf1)
    compss_barrier()
    t4 = time.time()
    print("Time to PCA.transform (t4-t3):", t4 - t3)
    print("t_all:", t4 - t1)
